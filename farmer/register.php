...
$fname = $farm =$email = $telephone = $address = $postcode = '';

$error = array('fname' => '','farm' => '', 'email' => '', 'password' => '', 'postcode' => '', 'cpassword' => '', 'telephone' => '', 'address' => '');
...
if (isset($_POST['register'])) {
    ...
    if (empty($farm)) {
        $error['farm'] = '‼️ Please name your farm';
    } else {
        $farm = htmlspecialchars($_POST['farm']);
    }
    ...
    if (empty($postcode)) {
        $error['postcode'] = '‼️ Please enter your post code';
    } else {
        $postcode = htmlspecialchars($_POST['postcode']);
    }
    ...
    if (array_filter($error)) {
        ...
        $sql1 = "SELECT * FROM farmertb WHERE email = '$email'";
        ...
        if ($count > 0) {
            ...
            $sql = "INSERT INTO farmertb (fname,farm,email,telephone,address,postcode,password) VALUES ('$fname','$farm','$email','$telephone','$address','$postcode','$password')";

            if (mysqli_query($conn, $sql)) {
                //Sql to check wether the user exist or not
                $sql1 = "SELECT * FROM farmertb WHERE email = '$email' AND password = '$password'";
    ...
    <div class="register-container">
        ...
        <form class="register-form" method="POST" action="">
            <div class="name-email">
                ...
                <div class="form-group">
                    <p class="farm-label" id="farmLabel">Farm Name</p>
                    <input type="text" id="farm" name="farm" class="farm" placeholder="Farm Name*" value="<?php echo $farm ?>">
                    <div class="errormsg"><b><?php echo $error['farm'] ?></b></div>
                </div>
            ...
            <div class="name-email">
            <div class="form-group" style="flex:1.7;" >
                ...
                <div class="form-group">
                        <p class="postcode-label" id="postcodeLabel">Post Code</p>
                        <input type="text" id="postcode" name="postcode" class="postcode" placeholder="Post Code*" value="<?php echo $postcode ?>">
                        <div class="errormsg"><b><?php echo $error['postcode'] ?></b></div>
                    </div>
            </div>
        ...
        <script>
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                var farmLabel = document.getElementById('farmLabel');

                farmInput.addEventListener('focus', function() {
                    farmLabel.style.display = 'block';
                });

                farmInput.addEventListener('blur', function() {
