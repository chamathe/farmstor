...
if($farmerId === $farmerunique){
   ...
   if ($countchek > 0) {
       ...
       if (isset($categoryId) && $categoryId !== "") {
        $location = "product.php?product=$product_id_cart&category_id=$categoryId";
    } else {
...
$sqlcartcount = "SELECT SUM(quantity) AS total_quantity FROM carttb WHERE buyer_id = '$buyer_id'";

$sqlitemcount = "SELECT * FROM carttb WHERE buyer_id = '$buyer_id'";

$resitemcount = mysqli_query($conn, $sqlitemcount);

$rescartcount = mysqli_query($conn, $sqlcartcount);

$rowscount = mysqli_fetch_assoc($rescartcount);

$item_count = mysqli_num_rows($resitemcount);

$total_quantity = $rowscount['total_quantity'];
